Напиши целиком файл `api/index.py` — ЕДИНЫЙ ASGI-entrypoint для Vercel (Python
runtime). Он объединяет два эндпоинта в одном ASGI-приложении `app`:
  - POST /api/telegram        — вебхук Telegram
  - GET  /api/exchange_token  — OAuth-callback Strava
Никаких фреймворков (без FastAPI/Starlette) — чистый ASGI-callable на stdlib.
Приложение работает в event-loop Vercel, поэтому НЕ используй asyncio.run —
эндпоинт-корутины просто await'ятся.

В начале файла обеспечь импорт пакета `bot` (файл в api/, пакет в корне репо):
```
import os
import sys
...
Импорты:
`import json`, `from urllib.parse import parse_qs`, `import aiohttp`,
